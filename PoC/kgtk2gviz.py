...
def kgtk2gviz (node, query, question, path):

    dir = "/app/kgtk/data/WD_PoC/"
    g_name = node +"-"+ query
    ...
    for file in os.listdir(dir):
        if query in file and node in file and ((path=="No" and "path" not in file) or (path=="Yes" and "path" in file)):
            ...
            for i in kgtk.index:
                ...
                with e.subgraph() as e1:
                    e1.attr(rank='same')
                    
                    e1.attr('node', shape='ellipse', fontsize='10', color= '', style= '', fontcolor = "black")                 
                    if "node1;label" in cols:
                        e1.node(kgtk["node1"][i].replace(":", "_"), label= kgtk["node1;label"][i])
                    else:
                        e1.node(kgtk["node1"][i].replace(":", "_"), label= "")
                    
                    if "node2;label" in cols:
                        if pd.isna(kgtk["node2;label"][i]):
                            e1.attr('node', shape='plain', fontsize='10', color= '', style= '', fontcolor = "forestgreen")
                            e1.node(kgtk["id"][i]+kgtk["node2"][i].replace(":", "_"), label= kgtk["node2"][i])	
                        else:
                            e1.node(kgtk["node2"][i].replace(":", "_"), label= kgtk["node2;label"][i])
                    
                    e1.attr('node', shape='point', fontsize='10', color= 'orange', style= 'filled')
                    if "id" in cols:
                        e1.node(kgtk["id"][i].replace(":", "_"), label= "")
...
for node in clist:
    for query, question in qdict.items():
        kgtk2gviz (node, query, question, path)
